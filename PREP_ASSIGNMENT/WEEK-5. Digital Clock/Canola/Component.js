...
export default function componentFactory(generateTemplate) {
  ...
  Component.prototype.clickEvent = function() {
    const eventData = generateTemplate.events;
    const eventKeys = Object.keys(eventData);
    const eventFunctions = Object.values(eventData);

    for (let i = 0; i < eventKeys.length; i++) {
      this.$target = this.$el.querySelector(eventKeys[i]);
      this.$target.addEventListener("click", eventFunctions[i]);
    }
  ...
  Component.prototype.updateClock = function () {

    const $timer = this.$el.querySelector(".update-time");

    if ($timer === null) {
      throw new CanolaError(`'${options.time}' 존재하지 않는 엘레먼트입니다.`);
    }

    function updateCurrentTime () {
      $timer.textContent = getTimeStamp();
    }

    setInterval(updateCurrentTime.bind(this), 1000);
